...
import {
  ...
  ActivityIndicator, FlatList, StyleSheet, Platform,
} from 'react-native';
...
type ResultadoSQL = { colunas: string[]; linhas: Record<string, any>[]; total_linhas: number; limitado: boolean };
...
// ─── Sub-componentes de Layout de Questão ────────────────────────────────────
...
/** Layout ENEM — cabeçalho de prova, numeração destacada */
...
/** Layout Vestibular — borda lateral colorida, fonte serifada */
...
// ─── Aba 1: Visualizador de Questões ─────────────────────────────────────────

function AbaVisualizador() {
  const { isWide, maxW, fs } = useLayout();
      ...
      <View style={{ width: '100%', maxWidth: maxW ?? 860 }}>
        ...
        <View style={s.filtrosBox}>
          ...
          <XStack gap={8} mb={12} flexWrap="wrap">
            {LAYOUTS.map((l) => (
              <TouchableOpacity key={l} onPress={() => setLayout(l)}
                style={[s.chip, layout === l && s.chipAtivo]}>
          ...
          <XStack gap={8} mb={12} flexWrap="wrap">
            {DIFICULDADES.map((d) => (
              <TouchableOpacity key={String(d.value)} onPress={() => setDif(d.value)}
                style={[s.chip, dificuldade === d.value && s.chipAtivo]}>
...
const s = StyleSheet.create({
  ...
  secao:         { fontSize: 11, color: '#aaa', marginTop: 8 },

...
import { ScrollView,KeyboardAvoidingView, Platform,  Alert, View, Text } from 'react-native';
...
export default function SettingsScreen() {
  ...
  const handleSave = async () => {
    ...
    const emailAlterado = email.trim().toLowerCase() !== authEmail?.trim().toLowerCase();
  ...
  const confirmDeletionStep2 = () => {
    ...
    Alert.alert(
      ...
      [
        ...
        { text: "EXCLUIR TUDO", style: "destructive", onPress: finalDeleteAction }
      ]
    );
  };
  ...
  const finalDeleteAction = async () => {
    try {
      const response = await api.delete('/users/delete-me', { data: { password: deletePassword } });
  ...
  >
    {/* Header */}
    <XStack alignItems="center" padding="$4" space="$2" backgroundColor="#fff" borderBottomWidth={1} borderBottomColor="#eee">
      <Button 
        ...
        chromeless 
      />
      <Text style={{ fontSize: 18, fontWeight: 'bold', color: '#000' }}>Configurações</Text>
    </XStack>
        ...
        <Card bordered padding="$4" backgroundColor="#fff">
          <YStack space="$2">
            <XStack space="$2" alignItems="center">
              ...
              <Text style={{ fontWeight: '600', color: '#000' }}>Dados Pessoais</Text>
            </XStack>
        ...
        <Card bordered padding="$4" backgroundColor="#fff">
          <YStack space="$3">
            <XStack space="$2" alignItems="center">
              ...
              <Text style={{ fontWeight: '600', color: '#000' }}>Segurança (Opcional)</Text>
            </XStack>
      ...
      <View style={{ 
        ...
        >
          <Card bordered padding="$5" backgroundColor="white" elevate>
            <YStack space="$4">
              <Text style={{ fontSize: 18, fontWeight: 'bold', color: '#FF3B30' }}>Confirmar Exclusão</Text>
              <Text style={{ color: '#666' }}>Digite sua senha atual para autorizar a exclusão da conta.</Text>
              ...
              <XStack space="$2" justifyContent="flex-end">
                <Button onPress={() => { setShowDeleteModal(false); setDeletePassword(''); }}>
                  Cancelar
                </Button>
                <Button backgroundColor="#FF3B30" color="white" onPress={confirmDeletionStep2}>
                  Confirmar
                </Button>
              </XStack>
            </YStack>

...
function CartaoAluno({ aluno, pos }: { aluno: AlunoStat; pos: number }) {
  ...
  return (
    <View style={s.cartao}>
      <TouchableOpacity onPress={carregando ? undefined : toggle} activeOpacity={0.8}>
...
function AbaRanking({ alunos }: { alunos: AlunoStat[] }) {
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 16 }}>
      {top.map((a, i) => <CartaoAluno key={a.id} aluno={a} pos={i + 1} />)}
      {top.length === 0 && (
...
function AbaAlunos({ alunos }: { alunos: AlunoStat[] }) {
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        {filtrados.map((a, i) => <CartaoAluno key={a.id} aluno={a} pos={i + 1} />)}
        {filtrados.length === 0 && (
